...
async function processBlock(height, retries = 3, delay = 1000) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            const isTransientError = error instanceof Error && (error.message.includes("SSL") ||
...
export async function runIndexer(startHeight) {
    ...
    const indexedBlocks = await BlockFee.find({
        ...
        .sort({ block_height: 1 })
        .lean();
    const indexedHeights = new Set(indexedBlocks.map(b => b.block_height));
    const gaps = [];
